...
// determinism (recovery > resolved > pending close > b-stale > liquidate >
// refresh), and selected-asset fidelity for every asset-bearing plan.
#[cfg(all(kani, feature = "contracts"))]
#[kani::proof_for_contract(V16Core::select_auto_crank_plan)]
#[kani::solver(cadical)]
fn contract_check_select_auto_crank_plan() {
    let summary: ActionableSummaryV16 = kani::any();
    let close_slot: usize = kani::any();
