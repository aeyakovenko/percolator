...
fn closure_ledger_inv_prepare_counterparty_lien_create_delta() {
    ...
    if let Ok((b2, s2)) =
        V16Core::prepare_counterparty_lien_create_delta(b, s, current_slot, amount)
    {
        ...
        assert_eq!(
            b2.valid_liened_backing_num
                .checked_sub(b.valid_liened_backing_num),
            Some(amount)
        );
        assert_eq!(
            s2.valid_liened_backing_num
                .checked_sub(s.valid_liened_backing_num),
            Some(amount)
        );
        ...
        kani::cover!(
            amount > 1
                ...
                && b.valid_liened_backing_num > 1
                && s.valid_liened_backing_num > 1,
            "ledger closure covers partial nonzero counterparty-lien creation"
        );
    ...
    if let Ok((b2, s2)) = V16Core::prepare_counterparty_lien_terminal_release_delta(b, s, amount) {
        ...
        kani::cover!(
            amount > 1
                ...
                && b.fresh_unliened_backing_num > 1
                && s.valid_liened_backing_num > amount,
            "ledger closure covers terminal counterparty-lien release from expired bucket"
        );
        assert!(kani_ledger_inv(&b2, &s2, &r));
    }
}
    ...
    if let Ok((b2, s2)) = V16Core::prepare_counterparty_lien_consume_delta(b, s, amount) {
        ...
        assert_eq!(
            b2.consumed_liened_backing_num
                .checked_sub(b.consumed_liened_backing_num),
            Some(amount)
        );
        assert_eq!(
            s.valid_liened_backing_num
        ...
        assert_eq!(
            s2.spent_backing_num.checked_sub(s.spent_backing_num),
            Some(amount)
        );
        assert_eq!(
            s2.provider_receivable_num
                .checked_sub(s.provider_receivable_num),
            Some(amount)
        );
        kani::cover!(
            amount > 1
                ...
                && s.spent_backing_num > 1,
            "ledger closure covers partial nonzero counterparty-lien consume"
        );
        assert!(kani_ledger_inv(&b2, &s2, &r));
    }
}
...
fn closure_ledger_inv_prepare_insurance_lien_create_delta() {
    ...
    if let Ok((r2, s2)) = V16Core::prepare_insurance_lien_create_delta(r, s, amount) {
        ...
        assert_eq!(
            r2.valid_liened_insurance_num
                .checked_sub(r.valid_liened_insurance_num),
            Some(amount)
        );
        assert_eq!(
            r2.insurance_credit_reserved_num,
        ...
        assert_eq!(r2.consumed_insurance_num, r.consumed_insurance_num);
        assert_eq!(
            s2.valid_liened_insurance_num
                .checked_sub(s.valid_liened_insurance_num),
            Some(amount)
        );
        assert_eq!(
            s2.insurance_credit_reserved_num,
        ...
        kani::cover!(
            amount >= 2 * BOUND_SCALE
                && r.valid_liened_insurance_num > 1
                && r.impaired_liened_insurance_num > 1
                && r.insurance_credit_reserved_num
                    > r2.valid_liened_insurance_num + r2.impaired_liened_insurance_num,
            "ledger closure covers partial nonzero insurance-lien creation"
        );
    ...
    if let Ok((r2, s2)) = V16Core::prepare_insurance_lien_release_delta(r, s, amount) {
        ...
        kani::cover!(
            amount >= 2 * BOUND_SCALE
                ...
                && r.impaired_liened_insurance_num > 1
                && r.insurance_credit_reserved_num
                    > r.valid_liened_insurance_num + r.impaired_liened_insurance_num,
            "ledger closure covers partial nonzero insurance-lien release"
        );
        assert!(kani_ledger_inv(&b, &s2, &r2));
    }
}
...
fn closure_ledger_inv_prepare_insurance_lien_impair_delta() {
    ...
    if let Ok((r2, s2)) = V16Core::prepare_insurance_lien_impair_delta(r, s, amount) {
        ...
        assert_eq!(
            r2.impaired_liened_insurance_num
                .checked_sub(r.impaired_liened_insurance_num),
            Some(amount)
        );
        assert_eq!(
            r2.insurance_credit_reserved_num,
        ...
        assert_eq!(
            s2.impaired_liened_insurance_num
                .checked_sub(s.impaired_liened_insurance_num),
            Some(amount)
        );
        assert_eq!(
            s2.insurance_credit_reserved_num,
        ...
        kani::cover!(
            amount >= 2 * BOUND_SCALE
                ...
                && s.valid_liened_insurance_num > amount
                && r.impaired_liened_insurance_num > 1
                && s.impaired_liened_insurance_num > 1,
            "ledger closure covers partial nonzero insurance-lien impairment"
        );
        assert!(kani_ledger_inv(&b, &s2, &r2));
    }
}
