...
Terminal source realization is payout-monotone. If source backing pays `X` effective quote atoms against an original terminal face `F`, the exact receipt face is `F - X`; only `X * BOUND_SCALE` leaves the payout denominator. The larger inverse-rate source face consumed from live source-credit accounting MUST NOT be treated as forfeited terminal entitlement. This corrects and supersedes the historical realizable-limited/non-monotone characterization below, which allowed one atom of backing to erase an independent winner's residual claim.
...
The resolved-payout exclusion of `counterparty_backing_principal` from the junior pool is sound only because outstanding source-credit claims realize against their backing at terminal. The engine captures the payout snapshot while the account's full pre-conversion bound remains unreceipted, then: (1) RELEASES the account's pre-existing liens (un-pledge `valid_liened -> fresh_unliened`, returning the backing to Fresh); (2) CREATES-AND-CONSUMES a fresh lien against that now-unliened backing, atomically crediting `X` effective quote atoms to account capital; and (3) replaces the account's full prior bound with an exact receipt for `F - X`. It does NOT consume a released lien (the lien state machine forbids consuming a Released lien); release and create-and-consume act on different liens. The un-pledge step exists only so the realizable estimate matches what fresh consumption can actually deliver. Realization is conservation-exact (`counterparty_backing_principal -= X`, `C_tot += X`, `V` unchanged), so the payout snapshot does not depend on realization order, and only backing net of realizable claims is provider-recoverable. The insurance-backed branch balances symmetrically (`I -= X`, `C_tot += X`, transit `CloseInsuranceSpent`).

Realization EXTENT and no-double-pay are exact. Let `F` be the account's original terminal positive face and `X <= F` the effective quote atoms delivered by source backing. Source-credit accounting may consume an inverse-rate face greater than `X`, but terminal distribution removes exactly `X` from the claim: the receipt records `prior_bound_contribution_num = F * BOUND_SCALE`, `live_released_face_at_receipt = X`, and `terminal_positive_claim_face = F - X`. The ledger subtracts the full prior bound from `terminal_claim_bound_unreceipted_num`, adds `(F - X) * BOUND_SCALE` to `terminal_claim_exact_receipts_num`, and recomputes the rate. If `F - X = 0`, the ledger migration still occurs but the inert zero-face receipt is omitted. Thus the denominator falls by exactly `X * BOUND_SCALE`, source capital and receipt payout are disjoint, and `total_paid(account) <= F`. Realization is idempotent: a second realization finds the source claims consumed and no positive PnL, so it credits zero.

DISTRIBUTION CHARACTERIZATION: terminal payout is monotone in effective source support. Holding the payout snapshot and all other claims fixed, increasing source support from `X1` to `X2` cannot reduce the account's total source-capital-plus-receipt payout: each newly delivered source atom removes at most that same atom from the exact receipt face. Zero backing leaves the full face in the receipt pool; dust backing pays dust and leaves `F - dust` in the pool; full backing pays the full face and leaves no receipt claim. No caller can use a dust source reservation to donate an independent winner's claim to other junior claimants. Order-independence covers both the snapshot and this effective-face replacement. Stripping a claim that was realizable in Live by resolving the market is forbidden. If a deployment instead keeps backing principal inside `C_tot`, it MUST rewrite both the senior-stack relation and the realization steps as intra-`C_tot` transfers (loser `-X`, winner `+X`) and drop `counterparty_backing_principal` as a separate class, but it MUST NOT mix the two models. `settlement_rounding_residue_total` and `unallocated_protocol_surplus` are protocol-owned, non-user-claim value and MUST NOT be used as proof of account health, source-credit backing, insurance credit, hedge collateral, or payout entitlement unless explicitly moved through a balanced `TokenValueFlowProof` and all senior invariants remain true.
...
4. require `(exact receipt face + live_released_face_at_receipt) * BOUND_SCALE == prior_bound_contribution_num`; after source realization the live-released component is exactly the effective source payment;
5. replace the full prior unreceipted bound with the exact residual claim, reducing total payout weight by exactly `effective source payment * BOUND_SCALE`;
