...
The plan MUST be built from every active leg before any newly observed K/F delta is applied. Its canonical order is `(negative phase first, source_domain ascending, canonical leg_slot ascending)`. A plan entry's K/F delta and source domain MUST be recomputed immediately before application and MUST match its key. This is ordering, not netting: each negative delta reserves available capital in its own source domain, and a same-refresh positive delta cannot erase that loss before reservation. Positive face that existed before the refresh remains subject to the ordinary source-support and face-burn rules when a new loss is applied.
