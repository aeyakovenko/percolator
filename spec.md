...
5. **No global B pool or lock:** bankruptcy residual is charged only to the asset-side domain whose exposure generated it. An attributed asset-side bankruptcy h-lock MUST NOT block favorable operations on accounts whose active legs, source claims, and close ledger reference only other asset domains. Same-domain and unattributed h-locks MUST remain fail-closed. The durable h-lock summary distinguishes inactive, fully attributed, and unattributed states so this decision is bounded by account shape rather than total configured assets.
