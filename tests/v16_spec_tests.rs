...
#[test]
fn v16_liquidation_isolates_unrelated_asset_and_moves_only_fee_value() {
    let (mut header, mut markets) = market_fixture(2, 5);
    header.config.max_price_move_bps_per_slot = V16PodU64::new(100);
    header.config.liquidation_fee_bps = V16PodU64::new(100);
    header.config.min_liquidation_abs = V16PodU128::new(1);
    header.config.liquidation_fee_cap = V16PodU128::new(1);
    header.config.min_nonzero_mm_req = V16PodU128::new(2);
    header.config.min_nonzero_im_req = V16PodU128::new(3);
    header
        .config
        .try_to_runtime_shape()
        .unwrap()
    ...
    header.c_tot = V16PodU128::new(9);

    let mut account_header = account_fixture(2, 15);
    account_header.capital = V16PodU128::new(9);
    ...
    let target_before = markets[0].engine;
    let unrelated_market_before = markets[1];
    let unrelated_leg_before = account_header.legs[1];
    let out = {
        let mut market = MarketGroupV16ViewMut::new(&mut header, &mut markets);
        market
    ...
    assert!((1..=POS_SCALE).contains(&out.closed_q));
    assert_eq!(out.fee_charged, 1);
    assert_eq!(out.insurance_used, 0);
    assert_eq!(out.residual_booked, 0);
    assert_eq!(out.explicit_loss, 0);
    assert_eq!(header.vault.get(), 9);
    assert_eq!(header.c_tot.get(), 8);
    assert_eq!(header.insurance.get(), 1);
    assert_eq!(account_header.capital.get(), 8);
    assert_eq!(account_header.legs[1], unrelated_leg_before);
    assert_eq!(markets[1], unrelated_market_before);
    assert_eq!(
        markets[0].engine.asset.oi_eff_long_q.get(),
        target_before.asset.oi_eff_long_q.get() - out.closed_q
    );
    assert_eq!(
        markets[0].engine.asset.oi_eff_short_q.get(),
        target_before.asset.oi_eff_short_q.get() - out.closed_q
    );
    assert_eq!(
        account_header
            .health_cert
...
#[test]
fn v16_auto_crank_commits_recovery_before_unbookable_liquidation() {
    const RESIDUAL: u128 = 4;
    let (mut header, mut markets) = market_fixture(1, 100);
    let mut account_header = account_fixture(1, 40);
...
#[test]
fn v16_auto_crank_declares_recovery_before_bankrupt_multi_asset_liquidation() {
    const LOSS: u128 = 4;
    const CAPITAL: u128 = 1_000;
    let (mut header, mut markets) = market_fixture(2, 100);
    let mut account_header = account_fixture(2, 43);
    let mut peer_header = account_fixture(2, 44);
    MarketGroupV16ViewMut::new(&mut header, &mut markets)
        .deposit_domain_insurance_not_atomic(1, LOSS)
        .unwrap();
    ...
    market.deposit_not_atomic(&mut peer, CAPITAL).unwrap();
    market
        .execute_batch_with_fee_loss_stale_scoped_not_atomic(
            &mut account,
            &mut peer,
    ...
    account.header.pnl = V16PodI128::new(-((CAPITAL + LOSS) as i128));
    account.header.last_fee_slot = V16PodU64::new(now_slot);
    account.header.health_cert = HealthCertV16Account::from_runtime(&HealthCertV16 {
        ...
        certified_liq_deficit: LOSS + 2,
        certified_worst_case_loss: 2,
        cert_oracle_epoch: market.header.oracle_epoch.get(),
        cert_funding_epoch: market.header.funding_epoch.get(),
        cert_risk_epoch: market.header.risk_epoch.get(),
        cert_asset_set_epoch: market.header.asset_set_epoch.get(),
    ...
    assert!(
        summary.liquidatable && summary.recovery_eligible,
        "bankrupt multi-asset liquidation must be commit-safely recoverable: {summary:?}"
    );

    let vault_before = market.header.vault;
    let c_tot_before = market.header.c_tot;
    let insurance_before = market.header.insurance;
    let markets_before = [market.markets[0].engine, market.markets[1].engine];
    let account_before = *account.header;
    let result = market
        .permissionless_auto_crank_not_atomic(
            &mut account,
    ...
    assert_eq!(market.header.c_tot, c_tot_before);
    assert_eq!(market.header.insurance, insurance_before);
    assert_eq!(market.markets[0].engine, markets_before[0]);
    assert_eq!(market.markets[1].engine, markets_before[1]);
    assert_eq!(*account.header, account_before);
    market.validate_shape().unwrap();
    account.validate_with_market(&market.as_view()).unwrap();
}
