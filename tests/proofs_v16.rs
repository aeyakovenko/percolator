...
#[kani::unwind(48)]
#[kani::solver(cadical)]
fn proof_v16_public_nonzero_raw_oracle_target_update_is_selected_only() {
    let target_raw: u16 = kani::any();
    let oracle_epoch_raw: u8 = kani::any();
    let capital_raw: u8 = kani::any();
    let insurance_slack_raw: u8 = kani::any();
    let surplus_raw: u8 = kani::any();
    kani::assume((1..=10_000).contains(&target_raw));

    let target = u64::from(target_raw);
    ...
    header.oracle_epoch = V16PodU64::new(u64::from(oracle_epoch_raw));
    ...
    assert_eq!(market.validate_shape(), Ok(()));
    market
        .set_asset_raw_oracle_target_not_atomic(1, target)
        .unwrap();

    kani::cover!(
        target < 100 && capital > 0 && insurance_slack > 0,
        "nonzero asset accepts a lower target with funded senior state"
    );
    kani::cover!(
        target > 100 && surplus > 0,
        "nonzero asset accepts a higher target with junior slack"
    );
    kani::cover!(target == 100, "nonzero asset target update is idempotent");

    let changed = target != 100;
    let mut expected_header = header_before;
    expected_header.oracle_epoch =
        V16PodU64::new(header_before.oracle_epoch.get() + u64::from(changed));
    assert!(kani_eq_market_group_v16_header_account(
    ...
    expected_selected.engine.asset.raw_oracle_target_price = V16PodU64::new(target);
...
// Spec #19/#69 and CrankForward liveness: a full same-side reduction may retain
// a weighted zero-basis leg while its domain-loss barrier is active. Once that
// barrier is released, the retained leg is permissionless cleanup work even if
// the account's health certificate is otherwise current. This calls the actual
// production classifier over a valid persisted market/account, rather than
// restating its predicate in the harness.
#[kani::proof]
#[kani::unwind(24)]
#[kani::solver(cadical)]
...
// eligible for its normal owner-driven dematerialization path.
#[kani::proof]
#[kani::unwind(24)]
#[kani::solver(cadical)]
...
// The wrapper charges both backing-fee destinations to the same source domain.
// Independent provider/insurance amounts and zero branches are proven above;
// these four route theorems keep one symbolic nonzero amount and prove the full
// no-steal frame over a populated sibling side and unrelated asset.
fn assert_v16_public_backing_fee_isolates_other_domains<
    const ASSET_INDEX: usize,
    const TARGET_LONG: bool,
>() {
    let fee_raw: u8 = kani::any();
    kani::assume((1..=4).contains(&fee_raw));
    let fee = fee_raw as u128;
    let total_fee = 2 * fee;
    let margin_slack = 8u128;
    let capital = total_fee + margin_slack;
    let target_domain = ASSET_INDEX * 2 + usize::from(!TARGET_LONG);
    let target_backing = 4u128;
    let sibling_backing = 5u128;
    let backing_atoms = target_backing + sibling_backing;
    let (mut header, mut markets, mut account_header) = two_market_direct_view_fixture();
    let market_id = markets[ASSET_INDEX].engine.asset.market_id.get();
    let (target_bucket, target_source) = fresh_backing_domain_accounts(market_id, target_backing);
    let (sibling_bucket, sibling_source) =
        fresh_backing_domain_accounts(market_id, sibling_backing);
    if TARGET_LONG {
    ...
    header.vault = V16PodU128::new(capital + 30 + backing_atoms + 4);
    account_header.capital = V16PodU128::new(capital);
    account_header.health_cert = HealthCertV16Account::from_runtime(&HealthCertV16 {
        certified_equity: capital as i128,
    ...
    let charged = market
        .kani_charge_account_backing_fee_after_preflight_not_atomic(
            &mut account,
            target_domain,
            fee,
            target_domain,
    ...
    kani::cover!(fee == 1, "backing-fee routing covers the minimum fee");
    kani::cover!(fee > 1, "backing-fee routing covers a multi-atom fee");
    assert_eq!(charged, total_fee);

    let mut expected_header = header_before;
    expected_header.c_tot = V16PodU128::new(capital - total_fee);
    expected_header.insurance = V16PodU128::new(30 + fee);
    expected_header.backing_provider_earnings_total = V16PodU128::new(fee);
    expected_header.insurance_domain_budget_remaining_total = V16PodU128::new(30 + fee);
    assert!(kani_eq_market_group_v16_header_account(
        &expected_header,
        market.header
    ));
    ...
    if TARGET_LONG {
        let mut bucket = expected_selected.backing_long.try_to_runtime().unwrap();
        bucket.utilization_fee_earnings = fee;
        expected_selected.backing_long = BackingBucketV16Account::from_runtime(&bucket);
        expected_selected.insurance_domain_budget_long = V16PodU128::new(fee);
    } else {
        let mut bucket = expected_selected.backing_short.try_to_runtime().unwrap();
        bucket.utilization_fee_earnings = fee;
        expected_selected.backing_short = BackingBucketV16Account::from_runtime(&bucket);
        expected_selected.insurance_domain_budget_short = V16PodU128::new(fee);
    }
    assert!(kani_eq_engine_asset_slot_v16_account(
        &expected_selected,
    ...
    expected_account.health_cert.certified_equity = V16PodI128::new(margin_slack as i128);
    assert!(kani_eq_portfolio_account_v16_account(
        &expected_account,
        account.header
    ));
    assert_eq!(
        market.header.c_tot.get()
            + market.header.insurance.get()
            + market.header.backing_provider_earnings_total.get(),
        header_before.c_tot.get()
            + header_before.insurance.get()
...
#[kani::unwind(64)]
#[kani::solver(cadical)]
fn proof_v16_production_asset_zero_long_backing_fee_core_isolates_other_domains() {
    assert_v16_public_backing_fee_isolates_other_domains::<0, true>();
}

#[kani::proof]
#[kani::unwind(64)]
#[kani::solver(cadical)]
fn proof_v16_production_asset_zero_short_backing_fee_core_isolates_other_domains() {
    assert_v16_public_backing_fee_isolates_other_domains::<0, false>();
}

#[kani::proof]
#[kani::unwind(64)]
#[kani::solver(cadical)]
fn proof_v16_production_asset_one_long_backing_fee_core_isolates_other_domains() {
    assert_v16_public_backing_fee_isolates_other_domains::<1, true>();
}

#[kani::proof]
#[kani::unwind(64)]
#[kani::solver(cadical)]
fn proof_v16_production_asset_one_short_backing_fee_core_isolates_other_domains() {
    assert_v16_public_backing_fee_isolates_other_domains::<1, false>();
}
...
fn check_v16_public_counterparty_backing_deposit_moves_vault_and_scaled_source_state<
    const TARGET_LONG: bool,
>() {
    // Broad add arithmetic is proved by the scalar delta theorem. This public
    // path theorem keeps the two economically distinct states with one Boolean:
    // first deposit into an empty domain/exactly empty junior pool, or additive
    // deposit beside live principal/nonzero junior value.
    let additive: bool = kani::any();
    let existing = if additive { 3u128 } else { 0u128 };
    let surplus = if additive { 4u128 } else { 0u128 };
    let amount = 2u128;
    ...
    let market_id = markets[0].engine.asset.market_id.get();
    header.c_tot = V16PodU128::new(7);
    header.vault = V16PodU128::new(7 + existing + surplus);
    if existing != 0 {
    ...
    market
        .deposit_fresh_counterparty_backing_not_atomic(target_domain, amount, 10)
        .unwrap();
    let (
        bucket_after,
        source_after,
    ...
    let expected_bucket = BackingBucketV16Account::from_runtime(&BackingBucketV16 {
        market_id,
        fresh_unliened_backing_num: existing_num + amount_num,
        expiry_slot: 10,
        status: BackingBucketStatusV16::Fresh,
        ..BackingBucketV16::EMPTY
    });
    let expected_source = SourceCreditStateV16Account::from_runtime(&SourceCreditStateV16 {
        fresh_reserved_backing_num: existing_num + amount_num,
        credit_rate_num: CREDIT_RATE_SCALE,
        credit_epoch: 1,
        ..SourceCreditStateV16::EMPTY
    });
    ...
    kani::cover!(
        !additive,
        "public backing deposit covers first principal with no junior surplus"
    );
    kani::cover!(
        additive,
        "public backing deposit covers additive principal beside junior surplus"
    );
    ...
    let stock_exact = market.header.vault.get() == header_before.vault.get() + amount
        && market.header.c_tot.get() == header_before.c_tot.get()
        && market.header.insurance.get() == header_before.insurance.get()
        && market.header.backing_provider_earnings_total.get()
            == header_before.backing_provider_earnings_total.get()
        && market.header.source_fresh_backing_total_num.get() == existing_num + amount_num
    ...
    assert!(
        ...
        "public backing deposit must move only vault and selected-domain principal"
    );
}
...
fn proof_v16_public_counterparty_backing_deposit_moves_vault_and_scaled_source_state() {
    check_v16_public_counterparty_backing_deposit_moves_vault_and_scaled_source_state::<true>();
}

#[kani::proof]
#[kani::unwind(48)]
#[kani::solver(cadical)]
fn proof_v16_public_short_counterparty_backing_deposit_moves_vault_and_scaled_source_state() {
    check_v16_public_counterparty_backing_deposit_moves_vault_and_scaled_source_state::<false>();
}
...
fn proof_v16_residual_excludes_recoverable_counterparty_backing_principal() {
    // Public deposit and withdrawal each run full ledger/shape validation and
    // are proved above for both sides. Repeating both validations in one Kani
    // formula exceeds the per-harness budget, so this theorem composes their
    // exact stock deltas over the real production residual formula.
    let c_tot_raw: u8 = kani::any();
    let insurance_raw: u8 = kani::any();
    let earnings_raw: u8 = kani::any();
    let backing_raw: u8 = kani::any();
    ...
    let backing_num = backing * BOUND_SCALE;
    let deposit_num = deposit * BOUND_SCALE;
    let (mut header, mut markets) = one_market_only_fixture();
    header.c_tot = V16PodU128::new(c_tot);
    header.insurance = V16PodU128::new(insurance);
    header.backing_provider_earnings_total = V16PodU128::new(earnings);
    header.source_fresh_backing_total_num = V16PodU128::new(backing_num);
    header.vault = V16PodU128::new(c_tot + insurance + earnings + backing + surplus);
    let market = MarketGroupV16ViewMut::new(&mut header, &mut markets);
    let residual_before = market.kani_residual();

    // Exact public deposit delta: external vault atoms and scaled recoverable
    // principal rise in lockstep. No other senior or junior class moves.
    market.header.vault = V16PodU128::new(market.header.vault.get() + deposit);
    market.header.source_fresh_backing_total_num = V16PodU128::new(backing_num + deposit_num);
    let residual_while_live = market.kani_residual();

    // Exact public withdrawal delta reverses those two stock movements.
    market.header.vault = V16PodU128::new(market.header.vault.get() - deposit);
    market.header.source_fresh_backing_total_num = V16PodU128::new(backing_num);
    let residual_after_withdrawal = market.kani_residual();

    let pre_reconciliation = StockReconciliationProofV16 {
        token_vault: c_tot + insurance + earnings + backing + surplus,
        senior_capital_total: c_tot,
        insurance_capital: insurance,
        backing_provider_earnings: earnings,
        counterparty_backing_principal: backing,
        settlement_rounding_residue_total: 0,
        unallocated_protocol_surplus: residual_before,
    };
    let live_reconciliation = StockReconciliationProofV16 {
        token_vault: c_tot + insurance + earnings + backing + deposit + surplus,
        senior_capital_total: c_tot,
        insurance_capital: insurance,
        backing_provider_earnings: earnings,
        counterparty_backing_principal: backing + deposit,
        settlement_rounding_residue_total: 0,
        unallocated_protocol_surplus: residual_while_live,
    };

    kani::cover!(
        c_tot > 4 && insurance > 4 && earnings > 4 && backing > 4 && surplus > 4 && deposit > 4,
        "principal exclusion composes across all nontrivial stock classes"
    );
    assert!(
        residual_before == surplus
            && residual_while_live == surplus
            && residual_after_withdrawal == surplus
            && pre_reconciliation.validate() == Ok(())
            && live_reconciliation.validate() == Ok(()),
        "lockstep backing principal cannot enter or consume junior residual"
    );
}
...
fn proof_v16_market_validator_reconciles_counterparty_backing_and_credit_rate() {
    ...
    assert!(source.spent_backing_num >= source.provider_receivable_num);
    assert_eq!(
        market.header.source_fresh_backing_total_num.get(),
        source.fresh_reserved_backing_num
    );
    ...
    let available_num = source.fresh_reserved_backing_num - source.valid_liened_backing_num;
    let expected_rate_num = (available_num * CREDIT_RATE_SCALE / claim_num).min(CREDIT_RATE_SCALE);
    assert_eq!(source.credit_rate_num, expected_rate_num);
}
