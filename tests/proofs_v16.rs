...
#[kani::unwind(4)]
#[kani::solver(cadical)]
fn proof_v16_batch_accumulator_three_fill_fold_is_permutation_independent() {
    let fill_count: u32 = kani::any();
    let fee_a: u128 = kani::any();
    let fee_b: u128 = kani::any();
    let notional: u128 = kani::any();
    ...
    let fee_b1: u128 = kani::any();
    let notional1: u128 = kani::any();
    let fee_a2: u128 = kani::any();
    let fee_b2: u128 = kani::any();
    let notional2: u128 = kani::any();
    let risk_before: bool = kani::any();
    let long_claim_before: bool = kani::any();
    let short_claim_before: bool = kani::any();
    let risk0: bool = kani::any();
    ...
    let long_claim1: bool = kani::any();
    let short_claim1: bool = kani::any();
    let risk2: bool = kani::any();
    let long_claim2: bool = kani::any();
    let short_claim2: bool = kani::any();

    kani::assume(fill_count <= u32::MAX - 3);
    kani::assume(fee_a0 <= u128::MAX - fee_a1);
    kani::assume(fee_b0 <= u128::MAX - fee_b1);
    kani::assume(notional0 <= u128::MAX - notional1);
    let fee_a01 = fee_a0 + fee_a1;
    let fee_b01 = fee_b0 + fee_b1;
    let notional01 = notional0 + notional1;
    kani::assume(fee_a01 <= u128::MAX - fee_a2);
    kani::assume(fee_b01 <= u128::MAX - fee_b2);
    kani::assume(notional01 <= u128::MAX - notional2);
    let fee_a_add = fee_a01 + fee_a2;
    let fee_b_add = fee_b01 + fee_b2;
    let notional_add = notional01 + notional2;
    kani::assume(fee_a <= u128::MAX - fee_a_add);
    kani::assume(fee_b <= u128::MAX - fee_b_add);
    kani::assume(notional <= u128::MAX - notional_add);
    ...
    let mut outcome_012 = initial;
    let mut risk_012 = risk_before;
    let mut long_claim_012 = long_claim_before;
    let mut short_claim_012 = short_claim_before;
    let mut outcome_201 = initial;
    let mut risk_201 = risk_before;
    let mut long_claim_201 = long_claim_before;
    let mut short_claim_201 = short_claim_before;
    ...
    kani::cover!(
        fee_a0 > 0 && fee_a1 > 0 && fee_a2 > 0 && notional0 > 0 && notional1 > 0 && notional2 > 0,
        "three-fill batch fold covers nonzero fees and notionals on all fills"
    );
    kani::cover!(
        risk0 && !risk1 && risk2 && long_claim0 && !long_claim1 && short_claim2,
        "three-fill batch fold covers mixed risk and source-claim flags"
    );

    assert_eq!(outcome_012, outcome_201);
    assert_eq!(risk_012, risk_201);
    assert_eq!(long_claim_012, long_claim_201);
    assert_eq!(short_claim_012, short_claim_201);
    assert_eq!(outcome_012.fill_count, fill_count + 3);
    assert_eq!(outcome_012.fee_a, fee_a + fee_a_add);
    assert_eq!(outcome_012.fee_b, fee_b + fee_b_add);
    assert_eq!(outcome_012.notional, notional + notional_add);
    assert_eq!(risk_012, risk_before || risk0 || risk1 || risk2);
    assert_eq!(
        long_claim_012,
        long_claim_before || long_claim0 || long_claim1 || long_claim2
    );
    assert_eq!(
        short_claim_012,
        short_claim_before || short_claim0 || short_claim1 || short_claim2
    );
}
...
#[kani::unwind(8)]
#[kani::solver(cadical)]
fn proof_v16_batch_signed_fee_reconstruction_three_legs_matches_engine_aggregate() {
    let first_account_long0: bool = kani::any();
    let first_account_long1: bool = kani::any();
    let first_account_long2: bool = kani::any();
    let whole0_raw: u8 = kani::any();
    let whole1_raw: u8 = kani::any();
    let whole2_raw: u8 = kani::any();
    let dust0_raw: u16 = kani::any();
    let dust1_raw: u16 = kani::any();
    let dust2_raw: u16 = kani::any();
    let price0_raw: u8 = kani::any();
    let price1_raw: u8 = kani::any();
    let price2_raw: u8 = kani::any();
    let fee_bps0_raw: u16 = kani::any();
    let fee_bps1_raw: u16 = kani::any();
    let fee_bps2_raw: u16 = kani::any();
    kani::assume(whole0_raw <= 4);
    kani::assume(whole1_raw <= 4);
    kani::assume(whole2_raw <= 4);
    kani::assume(dust0_raw <= 32);
    kani::assume(dust1_raw <= 32);
    kani::assume(dust2_raw <= 32);
    kani::assume(price0_raw > 0 && price0_raw <= 16);
    kani::assume(price1_raw > 0 && price1_raw <= 16);
    kani::assume(price2_raw > 0 && price2_raw <= 16);
    kani::assume(fee_bps0_raw <= 10_000);
    kani::assume(fee_bps1_raw <= 10_000);
    kani::assume(fee_bps2_raw <= 10_000);

    let size0 = whole0_raw as u128 * POS_SCALE + dust0_raw as u128;
    let size1 = whole1_raw as u128 * POS_SCALE + dust1_raw as u128;
    let size2 = whole2_raw as u128 * POS_SCALE + dust2_raw as u128;
    kani::assume(size0 > 0);
    kani::assume(size1 > 0);
    kani::assume(size2 > 0);
    let signed0 = if first_account_long0 {
    ...
    let signed2 = if first_account_long2 {
        size2 as i128
    } else {
        -(size2 as i128)
    };
    ...
    let (abs1, delta_a1, delta_b1) =
        MarketGroupV16ViewMut::<u64>::kani_trade_signed_size_deltas(signed1).unwrap();
    let (abs2, delta_a2, delta_b2) =
        MarketGroupV16ViewMut::<u64>::kani_trade_signed_size_deltas(signed2).unwrap();
    let price0 = price0_raw as u64;
    let price1 = price1_raw as u64;
    let price2 = price2_raw as u64;
    let fee_bps0 = fee_bps0_raw as u64;
    let fee_bps1 = fee_bps1_raw as u64;
    let fee_bps2 = fee_bps2_raw as u64;
    let notional0 = kani_trade_notional_floor(abs0, price0).unwrap();
    let notional1 = kani_trade_notional_floor(abs1, price1).unwrap();
    let notional2 = kani_trade_notional_floor(abs2, price2).unwrap();
    let fee0 = kani_trade_fee_atoms_per_side(abs0, price0, fee_bps0).unwrap();
    let fee1 = kani_trade_fee_atoms_per_side(abs1, price1, fee_bps1).unwrap();
    let fee2 = kani_trade_fee_atoms_per_side(abs2, price2, fee_bps2).unwrap();
    ...
    let fee_sum = fee0.checked_add(fee1).unwrap().checked_add(fee2).unwrap();
    let reconstructed_total = fee_sum.checked_add(fee_sum).unwrap();
    let engine_total = outcome.fee_a.checked_add(outcome.fee_b).unwrap();

    kani::cover!(
        first_account_long0 != first_account_long1
            && first_account_long1 != first_account_long2
            && fee0 > 0
            && fee1 > 0
            && fee2 > 0,
        "three-leg fee reconstruction covers alternating signed spread with positive fees"
    );
    kani::cover!(
        whole2_raw == 0 && dust2_raw > 0 && fee_bps2_raw > 0 && fee2 > 0,
        "three-leg fee reconstruction covers sub-atom third fill with nonzero fee"
    );
    kani::cover!(
        fee_bps1_raw == 0 && fee1 == 0 && fee0 > 0 && fee2 > 0,
        "three-leg fee reconstruction covers zero-fee middle leg beside fee-paying legs"
    );

    assert_eq!(abs0, size0);
    assert_eq!(abs1, size1);
    assert_eq!(abs2, size2);
    assert_eq!(delta_a0.checked_add(delta_b0), Some(0));
    assert_eq!(delta_a1.checked_add(delta_b1), Some(0));
    assert_eq!(delta_a2.checked_add(delta_b2), Some(0));
    assert_eq!(outcome.fill_count, 3);
    assert_eq!(outcome.fee_a, fee_sum);
    assert_eq!(outcome.fee_b, fee_sum);
    assert_eq!(outcome.notional, notional0 + notional1 + notional2);
    assert_eq!(engine_total, reconstructed_total);
    assert_eq!(long_has_source_claims, false);
    assert_eq!(short_has_source_claims, false);
    assert_eq!(risk_increasing, true);
...
fn proof_v16_backing_utilization_rate_above_kink_matches_exact_schedule() {
    ...
    kani::cover!(
        fresh > 32 && valid > 32 && util_bps > kink && valid < fresh,
        "backing utilization above-kink proof covers broader partial above-kink utilization"
    );
    kani::cover!(
...
fn proof_v16_backing_utilization_collection_full_charge_conserves_senior_value() {
    ...
    kani::cover!(
        slack_raw > 8 && earnings_raw > 8 && revenue_raw > 8,
        "backing-utilization full charge covers nontrivial existing provider and account revenue"
    );
...
#[kani::unwind(48)]
#[kani::solver(cadical)]
fn proof_v16_forfeit_dead_leg_negative_kf_delta_books_loss_without_positive_claim() {
    let loss_units_raw: u16 = kani::any();
    let support_raw: u16 = kani::any();
    kani::assume((1..=511).contains(&loss_units_raw));
    kani::assume(support_raw <= loss_units_raw);

    let loss_units = loss_units_raw as u128;
    let support = support_raw as u128;
    let (mut header, mut markets) = one_market_only_fixture();
    header.mode = 2;
    header.recovery_reason = V16OptionalRecoveryReasonAccount::from_runtime(Some(
    ...
    asset.k_long = 0;
    asset.k_short = 0;
    asset.f_long_num = -(loss_units as i128) * ADL_ONE as i128;
    asset.f_short_num = 0;
    markets[0].engine.asset = AssetStateV16Account::from_runtime(&asset);
    let leg = PortfolioLegV16 {
        active: true,
    ...
    let ledger = CloseProgressLedgerV16 {
        ...
        gross_loss_at_close_start: loss_settled,
        residual_remaining: loss_settled,
        ..CloseProgressLedgerV16::EMPTY
    };
    let support_proof =
        TokenValueFlowProofV16::support_to_account_capital(support, support, 0, 0, 21, 21).unwrap();
    let advanced = kani_kernel_advance_close_ledger(ledger, support, support, 0, 0, 0).unwrap();
    let settled_leg = PortfolioLegV16 {
        k_snap: k_now,
        f_snap: f_now,
        ..leg
    ...
    kani::cover!(
        support > 1 && support < loss_units,
        "dead-leg adverse forfeit covers partial support progress with residual remaining"
    );
    kani::cover!(
        support == loss_units && support > 1,
        "dead-leg adverse forfeit covers exact full support progress"
    );
    kani::cover!(
        support == 0 && loss_units > 1,
        "dead-leg adverse forfeit covers unsupported residual loss"
    );
    kani::cover!(
        dead && cleared_asset.oi_eff_long_q == 0 && cleared_asset.stored_pos_count_long == 0,
        "dead-leg adverse forfeit covers post-settlement exposure clear"
    );
    assert!(dead);
    assert_eq!(k_now, asset.k_long);
    assert_eq!(f_now, asset.f_long_num);
    assert_eq!(net, -(loss_units as i128));
    assert_eq!(loss_settled, loss_units);
    assert_eq!(positive_pnl_forfeited, 0);
    assert_eq!(support_proof.validate(), Ok(()));
    assert_eq!(
        support_proof.credits[TokenValueClassV16::CloseCounterpartyCreditConsumed as usize],
        support
    ...
    assert_eq!(advanced.support_consumed, support);
    assert_eq!(advanced.junior_face_burned, support);
    assert_eq!(advanced.residual_remaining, loss_units - support);
    assert_eq!(advanced.finalized, support == loss_units);
    assert_eq!(
        advanced.residual_remaining + support,
...
fn proof_v16_principal_then_domain_insurance_waterfall_is_loss_and_domain_capped() {
    ...
    assert_eq!(
        market.header.negative_pnl_account_count.get(),
        if expected_principal + expected_insurance == loss {
            0
        } else {
            1
        }
...
#[kani::unwind(40)]
#[kani::solver(cadical)]
fn proof_v16_counterparty_supported_external_exit_preserves_stock_reconciliation() {
    let amount_raw: u8 = kani::any();
    ...
    let initial_vault = initial_c_tot + insurance + junior;
    ...
    let reserve_proof = TokenValueFlowProofV16::account_capital_to_realized_loss(
        amount,
        initial_vault,
        initial_vault,
    )
    .unwrap();
    assert_eq!(reserve_proof.validate(), Ok(()));
    let reserved = StockReconciliationProofV16 {
        token_vault: initial_vault,
        senior_capital_total: initial_c_tot - amount,
        insurance_capital: insurance,
        backing_provider_earnings: 0,
        counterparty_backing_principal: amount,
        settlement_rounding_residue_total: 0,
        unallocated_protocol_surplus: junior,
    };
    assert_eq!(reserved.validate(), Ok(()));
    ...
    let final_vault = initial_vault - amount;
    let exit_proof =
        TokenValueFlowProofV16::account_capital_to_external_out(amount, initial_vault, final_vault)
            .unwrap();
    assert_eq!(exit_proof.validate(), Ok(()));
    let final_state = StockReconciliationProofV16 {
        token_vault: final_vault,
    ...
    assert_eq!(final_state.token_vault, initial.token_vault - amount);
    assert_eq!(
        final_state.senior_capital_total,
        initial.senior_capital_total - amount
    );
    assert_eq!(final_state.insurance_capital, initial.insurance_capital);
    assert_eq!(
        final_state.unallocated_protocol_surplus,
...
fn proof_v16_seq_two_fees_compound_exactly() {
    ...
    kani::cover!(
        capital > 128 && f1 > 32 && f2 > 32,
        "two-fee covers large nonzero fee composition"
    );
    assert_eq!(c1, f1);
    assert_eq!(c2, f2);
...
fn proof_v16_frame_earnings_withdraw_touches_only_declared_state() {
    ...
    kani::cover!(
        earn > 128 && amt < earn,
        "earnings withdraw frame covers large partial extraction"
    );
    let mut eh = h0;
...
// Permissionless no-DoS classifier/selector seam: if committed state already
// carries a CURRENT health certificate with open risk and a nonzero liquidation
// deficit, the engine must classify the account as liquidatable and select the
// liquidation continuation. A caller cannot strand the account by omitting an
// observation or by choosing a lower-priority refresh/no-action path.
#[kani::proof]
#[kani::unwind(40)]
#[kani::solver(cadical)]
fn proof_v16_auto_crank_classifier_current_deficit_open_risk_selects_liquidate() {
    ...
    kani::cover!(
        deficit > 1 && capital > 0 && insurance > 0,
        "auto-crank classifier covers current open-risk liquidation with nontrivial balances"
    );
    assert!(!summary.stale);
    assert!(!summary.b_stale);
    assert!(!summary.pending_close);
    ...
    assert!(!summary.recovery_eligible);
    assert!(!summary.resolved_winner);
    assert_eq!(plan, AutoCrankPlanV16::Liquidate { asset_index: 0 });
    assert!(!percolator::v16::auto_crank_plan_requires_caller_observation(&plan));
}

// Permissionless anti-LOF classifier seam: a nonzero liquidation deficit is not
// actionable for liquidation unless the health certificate is current for the
// committed account bitmap and market epochs. A stale deficit on an account with
// open risk must refresh first, so a keeper cannot force liquidation from an old
// certified deficit after the account or market state has moved.
#[kani::proof]
#[kani::unwind(40)]
#[kani::solver(cadical)]
fn proof_v16_auto_crank_classifier_stale_deficit_open_risk_selects_refresh_not_liquidate() {
    let deficit_raw: u16 = kani::any();
    let capital_raw: u16 = kani::any();
    let insurance_raw: u16 = kani::any();
    kani::assume(deficit_raw > 0);
    ...
    kani::cover!(
        deficit > 1 && capital > 0 && insurance > 0 && active_asset == Some(0),
        "auto-crank classifier covers stale open-risk deficit routed to committed-asset refresh"
    );
    assert!(summary.stale);
    assert!(!summary.b_stale);
    assert!(!summary.pending_close);
...
// already-committed B-index obligation can make progress.
#[kani::proof]
#[kani::unwind(40)]
#[kani::solver(cadical)]
fn proof_v16_auto_crank_classifier_b_stale_leg_selects_settle_b_without_observation() {
    let capital_raw: u16 = kani::any();
    let insurance_raw: u16 = kani::any();
    kani::assume(capital_raw <= 1024);
    kani::assume(insurance_raw <= 1024);
    ...
    kani::cover!(
        capital > 0 && insurance > 0,
        "auto-crank classifier covers current b-stale account with nontrivial balances"
    );
    assert!(!summary.stale);
    assert!(summary.b_stale);
    assert!(!summary.pending_close);
...
// Permissionless terminal-progress priority seam: an expired outstanding close
// residual must route to recovery even if the account also has a committed
// b-stale leg and a current liquidation deficit. This proves the real
// classifier/selector pair cannot keep sending an unadvanceable close account to
// lower-priority per-leg work.
#[kani::proof]
#[kani::unwind(40)]
#[kani::solver(cadical)]
fn proof_v16_auto_crank_classifier_expired_close_wins_over_b_stale_and_liquidation() {
    let residual_raw: u16 = kani::any();
    let deficit_raw: u16 = kani::any();
    let capital_raw: u16 = kani::any();
    let insurance_raw: u16 = kani::any();
    ...
    kani::cover!(
        residual > 1 && deficit > 1 && capital > 0 && insurance > 0 && b_stale_asset == Some(0),
        "auto-crank classifier covers expired close overlapping b-stale and liquidation"
    );
    assert!(!summary.stale);
    assert!(summary.b_stale);
    assert!(!summary.pending_close);
...
fn proof_v16_validator_sound_domain_budget_remaining_total_matches_slots() {
    ...
    kani::cover!(
        result.is_ok()
            ...
            && long_spent > 0
            && short_spent > 0,
        "domain-budget aggregate validator covers large two-domain spent budgets"
    );
    kani::cover!(
        result.is_ok()
            && !stale_header_total
    ...
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            && stale_header_total
            && expected_remaining > 128
            && insurance >= reported_remaining,
        "domain-budget aggregate validator rejects large stale header capacity despite global insurance"
    );
...
fn proof_v16_validator_sound_source_insurance_reserved_total_matches_slots() {
    ...
    kani::cover!(
        result.is_ok() && !stale_header_total && long_reserved > 128 && short_reserved > 128,
        "source-insurance aggregate validator covers large two-domain reservations"
    );
    kani::cover!(
        result.is_ok() && !stale_header_total && long_reserved > 0 && short_reserved == 0,
    ...
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            && stale_header_total
            && !stale_understates_slots
            && expected_reserved > 256,
        "source-insurance aggregate validator rejects large over-reported header reservation"
    );
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            && stale_header_total
    ...
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            ...
            && stale_understates_slots
            && expected_reserved > 256,
        "source-insurance aggregate validator rejects large under-reported header reservation"
    );
...
fn proof_v16_validator_sound_global_bound_covers_source_claim_total() {
    ...
    kani::cover!(
        result.is_ok()
            && source_claim_num > 128 * BOUND_SCALE
            && global_bound_num == source_claim_num,
        "global/source bound soundness covers large exact source-claim coverage"
    );
    kani::cover!(
        result.is_ok() && source_claim_num > 0 && global_bound_num > source_claim_num,
    ...
    kani::cover!(
        result.is_ok()
            && source_claim_num > 128 * BOUND_SCALE
            && global_bound_num > source_claim_num,
        "global/source bound soundness covers large slack source-claim coverage"
    );
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            && source_claim_num > 0
    ...
    kani::cover!(
        result == Err(V16Error::InvalidConfig)
            && source_claim_num > 128 * BOUND_SCALE
            && global_bound_num < source_claim_num,
        "global/source bound soundness covers large understated global-bound rejection"
    );
    if result.is_ok() {
