...
/// (the proof_* family doesn't have a counterpart for this operation)
#[kani::proof]
#[kani::unwind(33)]
#[kani::solver(cadical)]
...
fn proof_gap5_fee_credits_saturating_near_max() {
    ...
    // Non-vacuity: force Ok path — ensures assertions above are actually reached
    // Without this, if extreme fee_credits always causes trade failure, all assertions
    // would be vacuously true (never evaluated).
    let _ = assert_ok!(result, "trade must succeed near fee_credits max (non-vacuity)");
}
...
// ============================================================================
// NEW PROOFS: Symbolic Oracle Liquidation + Slot Reuse
// Added 2026-02-17 per Kani audit recommendations
// ============================================================================

/// Liquidation with symbolic oracle price — exercises mark-to-market settlement branch
///
/// The existing `proof_liquidate_preserves_inv` uses entry==oracle (mark_pnl=0),
/// which never exercises `settle_mark_to_oracle_best_effort`. This proof uses
/// symbolic oracle ∈ [800K, 1.2M] with entry=1M, exercising both profit and loss
/// mark-to-market paths during liquidation.
///
/// For long positions with capital=500 at entry=1M:
///   oracle < ~1.05M → user undercollateralized → liquidation triggers (mark ≠ 0)
///   oracle > ~1.05M → user overcollateralized → liquidation skipped (state unchanged)
/// Both paths must preserve canonical_inv.
#[kani::proof]
#[kani::unwind(5)] // MAX_ACCOUNTS=4
#[kani::solver(cadical)]
fn proof_liquidate_symbolic_oracle_preserves_inv() {
    let mut engine = RiskEngine::new(test_params());
    engine.current_slot = 100;
    engine.last_crank_slot = 100;
    engine.last_full_sweep_start_slot = 100;
    ...
    // Symbolic oracle price — exercises mark-to-market during liquidation
    let oracle_price: u64 = kani::any();
    kani::assume(oracle_price >= 800_000 && oracle_price <= 1_200_000);

    // Create user with long position at entry_price (undercollateralized for most oracle values)
    let user_idx = engine.add_user(0).unwrap();
    engine.accounts[user_idx as usize].capital = U128::new(500);
    engine.accounts[user_idx as usize].position_size = I128::new(5_000_000);
    ...
    // Create LP with counterparty short position (well-capitalized)
    let lp_idx = engine.add_lp([1u8; 32], [0u8; 32], 0).unwrap();
    ...
    // vault = user_capital + lp_capital + insurance
    engine.vault = U128::new(500 + 500_000 + 10_000);
    engine.insurance_fund.balance = U128::new(10_000);
    sync_engine_aggregates(&mut engine);
    ...
    // INV on Ok path — exercises mark-to-market settlement when oracle ≠ entry
    if result.is_ok() {
        kani::assert(
            canonical_inv(&engine),
            "INV must hold after liquidation with symbolic oracle (mark-to-market exercised)",
        );
    }
}
...
fn proof_slot_reuse_preserves_inv() {
    ...
    // NON-VACUITY: slot was actually reused (new user index == old user index)
    kani::assert(
        user2_idx == user1_idx,
